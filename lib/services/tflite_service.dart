...
import 'package:flutter_ml_applied_app/helpers/image_helper.dart';
...
class TfliteService {
  ...
  Future<void> initialize({String? customModelPath}) async {
    try {
      ...
      _isInitialized = true;
      appLogger.i(
        'TFLite initialized. Labels: ${_labels.length}, Model: $_modelFilePath',
      );
    } catch (e) {
  ...
  Future<List<PredictionResult>> runInference(String imagePath) async {
    ...
    final float32Data = Float32List.fromList(inputBytes.buffer.asFloat32List());
  ...
  List<PredictionResult> _processProbabilities(List<double> probabilities) {
    ...
    for (int i = 0; i < probabilities.length && i < _labels.length; i++) {
      results.add(
        PredictionResult(label: _labels[i], confidence: probabilities[i]),
      );
    }
  ...
  Future<List<PredictionResult>> runInferenceFromBytes(
    ...
    final float32Data = Float32List.fromList(imageBytes.buffer.asFloat32List());
